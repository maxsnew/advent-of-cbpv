...
(def-thunk (! parse-digit d)
  (! <<v swap - 48 'o char->integer d))

;; Listof ...  -> Listof (List Nat ModCol)
(def/copat (! parse-contents)
  [(c #:bind) (! parse-contents c '())]
  [((list _no _other _bags) acc) (ret acc)]
  [((cons (list d) (cons mod (cons col (cons _bags tl)))) acc)
   [num <- (! parse-digit d)]
   [mod <- (! list->string mod)] [col <- (! list->string col)]
   (! parse-contents tl (cons (list num (list mod col)) acc))]
  [('() acc) (ret acc)])

(def-thunk (! parse-line s)
  [words <- (! CBN (~! colist<-list s)
       % n> (~! sep-by #\space)
       % n> list<-colist
       % n$)]
  (pat words
    [(cons mod (cons col (cons _bags (cons _contain contents))))
     [mod <- (! list->string mod)] [col <- (! list->string col)]
     (! idiom^ (~! List (list mod col)) (~! parse-contents contents))]))

;; String -> F (Listof (List ModCol (Listof (List Nat ModCol))))
(def-thunk (! parse-inp s)
  (! CBN (~! colist<-string s)
     % n> (~! sep-by #\newline)
     % n> (~! cl-map parse-line)
     % n> list<-colist
...
;; (Listof (List ModCol (Listof (List Nat ModCol)))) -> FU Table String (Listof String)
(def-thunk (! can-be-contained-in-tbl inp)
  [step = (~ (copat [(tbl (list container containees))
   [add-edge = (~ (copat [(tbl (list count containee))
    (! update tbl containee
       (list container)
       (~! Cons container))]))]
   (! foldl containees add-edge tbl)]))]
  (! foldl inp step empty-table))

(def-thunk (! contains-tbl inp)
  [step = (~ (copat [(tbl (list container containees))
   [add-edge = (~ (copat [(tbl count*containee)
    (! update tbl container
       (list count*containee)
       (~! Cons count*containee))]))]
   (! foldl containees add-edge tbl)]))]
  (! foldl inp step empty-table))

(def-thunk (! reachable-loop tbl frontier known)
  [known? = (~! swap member? (~! colist<-list known))]
  [new-edges = (~ (λ (x) (! <<v filter (~! <<v not 'o known?) 'o tbl 'get x '())))]
  (patc (! idiom^ (~! apply append) (~! map new-edges frontier))
   ['() (ret known)]
   [frontier
    (! idiom^ (~! reachable-loop tbl frontier) (~! append frontier known))]))

(def-thunk (! reachable tbl start)
  (! CBV (~! reachable-loop tbl (list start) '())
     % v> list->set
     % v> set->list
     % v$))

(def-thunk (! main-a inp)
  [tbl <- (! idiom^ can-be-contained-in-tbl (~! parse-inp inp))]
