...
(def-thunk (! find-loop inp)
  (! handle (~! interp-game 0 0)
     ...
     (~ (copat
         [((list 'read-instr pc acc) k seen)
          ;; (! displayall 'debug pc acc seen)
...
(def/copat (! twiddle)
  [((list 'jmp n))
   (! choose
      (~! retE (list 'twiddled (list 'nop n)))
      (~! retE (list 'same     (list 'jmp n))))]
  [((list 'nop n))
   (! choose
      (~! retE (list 'twiddled (list 'jmp n)))
      (~! retE (list 'same     (list 'nop n))))]
  [(i) (! retE (list 'same i))])
...
(define! lex-sig
  (! idiom^ List
     (~! exact-string/lex "jmp" 'jmp)
     (~! exact-string/lex "nop" 'nop)
     (~! exact-string/lex "acc" 'acc)
     (~! exact-string/lex "\n" 'space)
     (~! exact-string/lex " " 'space)
     (~! Ret number/lex)))

(def-thunk (! parse-inp s)
  [l <- (! CBN (~! colist<-string s)
       ;; % n> (~! cl-map debug)
       % n> (~! fold-lex lex-sig)
       ;; % n> (~! cl-map debug)
...
(def-thunk (! main-a inp)
  (! idiom^ displayall (~! find-loop inp)))

(def-thunk (! main-b inp)
  (! idiom^ displayall (~! fix-loop inp)))

(do [full <- (! parse-inp full-string)]
    (! displayall 'parsed)
    (! main-a full)
    (! main-b full))
;; (! main-a)
