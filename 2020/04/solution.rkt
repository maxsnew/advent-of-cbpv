...
;;   CoList U(Table String String)

;; parse-one-tbl : U(CoList Char) -> F(List Tbl U(CoList Char))
(def-thunk (! parse-one-tbl cs tbl)
  (patc (! split-when (~! equal? #\:) cs) [(list key cs)
  [cs <- (! idiom^ clv-tl cs)]
  [key <- (! list->string key)]
  (patc (! split-when (~! swap member? (~! colist<-list '(#\space #\newline))) cs) [(list val cs)
  [cs <- (! idiom^ clv-tl cs)]
  [val <- (! list->string val)]
  [tbl <- (! tbl 'set key val)]
  (patc (! idiom^ view cs)
    ['() (! Cons tbl cl-nil)]
    [(cons #\newline cs) (! Cons tbl cs)]
    [(cons hd cs) (! parse-one-tbl (~! cl-cons hd cs) tbl)])])]))

;; parse-tbls-iter : U(CoList Char) -> Nil U (Cons Tbl U(CoList Char))
(def-thunk (! parse-tbls-iter c)
  (patc (! idiom^ view c)
    ['() (ret clv-nil)]
    [(cons hd tl)
     (! parse-one-tbl (~! cl-cons hd tl) empty-table)]))

(def-thunk (! parse-tbls c)
  (! cl-unfold parse-tbls-iter c))
...
(def-thunk (! correct-fields? tbl)
  (! CBV
     (~! tbl 'to-list) % v>
     (~! map car) % v>
     (~! swap sort string<=?) % v>
     (~ (λ (x) (! or (~! equal? x all-fields) (~! equal? x all-but-cid)))) % v$
     ))

(def-thunk (! correct-field-passports inp)
  (! CBN
     (~! colist<-string inp) % n>
     parse-tbls % n>
     (~! cl-filter correct-fields?) % n$))

(def-thunk (! main-a inp)
  (! CBN (~! correct-field-passports inp) % n> cl-length % n$))


(def-thunk (! n-digit-between n lo hi s)
  (! and
     (~! <<v equal? n 'o string-length s)
     (~! string<=? lo s)
     (~! string<=? s hi)))

(define colors '("amb" "blu" "brn" "gry" "grn" "hzl" "oth"))

(define rules
  (list
   (list "byr" (~! n-digit-between 4 "1920" "2002"))
   (list "iyr" (~! n-digit-between 4 "2010" "2020"))
   (list "eyr" (~! n-digit-between 4 "2020" "2030"))
   (list "hgt" (~ (λ (s)
                    (patc (! split-when (~! swap member? (~! colist<-string "ci")) (~! colist<-string s))
                          [(list front units)
                           [front <- (! list->string front)]
                           (patc (! list<-colist units)
                                 [(= '(#\c #\m)) (! n-digit-between 3 "150" "193" front)]
                                 [(= '(#\i #\n)) (! n-digit-between 2 "59" "76"   front)]
                                 [_ (ret #f)])]))))
   (list "hcl" (~ (λ (s)
                    (! and
                       (~! <<v equal? #\# 'o string-ref s 0)
                       (~! <<v equal? 7 'o string-length s)
                       (~ (! CBN
                              (~! colist<-string s) % n>
                              tl % n>
                             (~! cl-map (~! swap member? (~! colist<-string "0123456789abcdef"))) % n>
                             all? % n$))))))
   (list "ecl" (~! swap member? (~! colist<-list colors)))
   (list "pid" (~! n-digit-between 9 "000000000" "999999999"))
   ))

(def/copat (! apply-rule tbl)
  [((list key good?))
   (patc (! tbl 'get key #f)
     [#f (ret (list 'missing-key: key))]
     [val
      (cond [(! good? val) (ret #t)]
            [else (ret (list 'bad-key-val: key val))])])])

(def-thunk (! apply-rules tbl rules)
  (! map (~! apply-rule tbl) rules))

(def-thunk (! main-b inp)
    (! CBN
       (~! correct-field-passports inp) % n>
       (~! cl-map (~! swap apply-rules rules)) % n>
       (~! cl-filter (~! equal? '(#t #t #t #t #t #t #t))) % n>
       cl-length % n$))

(define invalid-inputs "eyr:1972 cid:100
...
(define valid-inputs "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
